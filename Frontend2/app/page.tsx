...
const FEATURES = [
  {
    ...
    desc: 'Vous pouvez choisir un document parmi nos documents disponibles',
  },
   {
    icon: '🎯',
    title: 'Remplir le Document',
    desc: 'Remplissez les champs du document.Votre Document se génère en temps réel pendant que vous saisissez.',
  },
  {
    icon: '📥',
    title: 'Export DOCX',
    desc: 'Téléchargez le document rempli au format Word ou Pdf, prêt à être envoyé ou imprimé directement.',
  },
]
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-cream font-sans">
      ...
      {/* ── HERO ── */}
      <section className="relative overflow-hidden text-white text-center flex items-center justify-center pt-[90px] pb-[140px] px-6 min-h-[680px] bg-brand">
        {/* Gradient overlay */}
        ...
        {/* Content */}
        <div className="w-full max-w-[1280px] mx-auto relative z-[2] px-6">
          <div
            className="animate-hero-badge inline-block mt-0 px-4 py-[5px] border border-white/25 rounded-[20px] text-[12px] font-bold text-white/90 uppercase tracking-[1.5px]"
            style={{ backgroundColor: 'rgba(255,255,255,0.12)' }}>
            Documents juridiques
          </div>
          <h1
            className="animate-hero-title w-full mb-5 text-white/80 font-extrabold"
          style={{ fontSize: 'clamp(36px, 6vw, 46px)', letterSpacing: '-1px' }}>
            Générez vos documents en toute simplicité
          </h1>
          <p
          ...
          <div className="animate-hero-btns flex gap-3.5 justify-center flex-wrap">
            <Link
              href="/documents"
              className="hero-btn-primary px-[34px] py-[14px] bg-white/75 text-brand no-underline rounded-lg font-bold text-base shadow-[0_4px_16px_rgba(0,0,0,0.15)] transition-[transform,box-shadow] duration-[180ms]"
            ...
            {templates.slice(0, 6).map((t, i) => (
              <div
                ...
                <div className="flex gap-2 shrink-0">
                  <Link
                    href={`/template/${t.id}`}
                    className="px-[14px] py-2 bg-white border border-gray-200 hover:bg-gray-50 text-brand no-underline rounded-md font-semibold text-[13px] whitespace-nowrap transition-colors">
                    Voir
                  </Link>
                  <Link
                    href={`/fill/${t.id}`}
      ...
      <footer className="bg-navy text-white/55 text-center py-8 px-6 text-[13px]">
        ...
        <p className="m-0 mb-3">Générateur de documents juridiques · {new Date().getFullYear()}</p>
        <div className="flex gap-6 justify-center">
          <Link href="/Contact" className="text-white/70 no-underline hover:text-white transition-colors">Contact</Link>
          <Link href="/faq" className="text-white/70 no-underline hover:text-white transition-colors">FAQ</Link>
          <Link href="/documents" className="text-white/70 no-underline hover:text-white transition-colors">Documents</Link>
        </div>
      </footer>
    </div>
  )
