...
function parseBorder(node) {
  if (!node) return null;
  const val = String(node['@_w:val'] || '').toLowerCase();
  if (!val || val === 'nil' || val === 'none') return null;
  const sz = Number(node['@_w:sz'] || 0);
  const color = String(node['@_w:color'] || '').toLowerCase();
  const cssWidth = Math.max(1, Math.round(sz / 8));
  return {
    style: val,
    width: cssWidth,
...
function extractPPr(pPr) {
  ...
  if (pPr['w:pBdr']) {
    const borders = {};
    const top = parseBorder(pPr['w:pBdr']['w:top']);
    const bottom = parseBorder(pPr['w:pBdr']['w:bottom']);
    const left = parseBorder(pPr['w:pBdr']['w:left']);
    const right = parseBorder(pPr['w:pBdr']['w:right']);
  ...
  // Explicit page break before paragraph (w:pageBreakBefore in paragraph properties)
  if (pPr['w:pageBreakBefore'] !== undefined) {
    const val = pPr['w:pageBreakBefore']?.['@_w:val'];
...
function extractRPr(rPr) {
  ...
  if (rPr['w:spacing']) {
    props.charSpacing = Number(rPr['w:spacing']['@_w:val'] ?? 0) / 20;
  }
